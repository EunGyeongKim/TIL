# 2022/08/31 프로그래머스_2019 카카오 개발_크레인 인형뽑기 게임
...
# 해당하는 인형 번호, 위치 return
def doll_check(board, location_j):
    location_j = location_j -1 
    shape_i, j = np.shape(board)
    for i in range(shape_i):
        if board[i][location_j] > 0:
            a = board[i][location_j]
            return board[i][location_j] , i
...
def solution(board, moves):
    ...
    for order in moves:
        # 해당하는 인형 번호, 위치  
        doll, location = doll_check(board, order)
        # 뽑은 인형들
        if doll > 0:
            insert_doll.append(doll)
            # print(insert_doll)
            if insert_doll[len(insert_doll)-1] == insert_doll[len(insert_doll)-2] and len(insert_doll) > 1:
                insert_doll.pop(len(insert_doll)-1)
                insert_doll.pop(len(insert_doll)-1)
                # print(insert_doll)
                answer = answer +2
        # 보드에서 뽑은 인형 0으로 변경
        board[location][order-1] = 0
